...
for segmentID = 1:nSegments
    ...
    if addMarkers == 1 || addMarkers == '1')
        ...
        if( isempty(functionHandleAddMarkersToSegment) == 0)
            [objectModel{segmentID}.marker_names, ...
                objectModel{segmentID}.marker_values] = ...
                functionHandleAddMarkersToSegment (objectModel, modelSegment_typeName,...
                humanAnthropometry, segmentID);
        else
            assert(0, ['Error: addMarkers is set to 1 but the function',...
                      ' handle functionHandleAddMarkersToSegment has not',...
                      ' been provided.']);
        end
%        [objectModel{segmentID}.marker_names, ...
%            objectModel{segmentID}.marker_values] = ...
